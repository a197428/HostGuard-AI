...
import {
  ...
  SupabaseRepository,
} from "@hostguard/backend/infrastructure/supabase";
import {
  RedisRepository,
  MockRedisRepository,
  REDIS_KEYS,
  REDIS_TTL,
} from "@hostguard/backend/infrastructure/redis";
...
describe("Redis Integration", () => {
  describe("Review Deduplication", () => {
    ...
    it("should detect already processed review", async () => {
      ...
      const isProcessed = await redis.isReviewProcessed(
        "ostrovok",
        "review_456",
      );
      expect(isProcessed).toBe(true);
    ...
    it("should check and mark atomically", async () => {
      ...
      const result1 = await redis.checkAndMarkReviewProcessed(
        "avito",
        "atomic_review",
      );
      ...
      const result2 = await redis.checkAndMarkReviewProcessed(
        "avito",
        "atomic_review",
      );
  ...
  describe("MockRedisRepository isolation", () => {
    it("should isolate data between tests", async () => {
      ...
      const inRedis1 = await redis1.isReviewProcessed(
        "avito",
        "review_in_redis1",
      );
      const inRedis2 = await redis2.isReviewProcessed(
        "avito",
        "review_in_redis1",
      );
