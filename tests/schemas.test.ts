...
describe('AI Response Schemas (from PROMPTS.md)', () => {
  describe('AppealAgentResponseSchema', () => {
    ...
    it('should accept appeal when violation_detected is true (business logic requirement)', () => {
      // The Zod schema allows appeal to be optional even when violation_detected is true
      // because this is a business logic requirement, not a schema constraint.
      // Business logic (in orchestrators.ts) should enforce that appeal is present
      // when violation_detected is true.
      const responseWithViolationButNoAppeal = {
      ...
      // Schema accepts this - business logic enforces the requirement
      const result = validateSchema(AppealAgentResponseSchema, responseWithViolationButNoAppeal);
      expect(result.violation_detected).toBe(true);
      expect(result.appeal).toBeUndefined();
    });
    ...
    it('should fail on invalid sentiment', () => {
      ...
      expect(() => validateSchema(AppealAgentResponseSchema, invalidResponse)).toThrow();
    });
  ...
  describe('Enums', () => {
    it.each([
      ...
      ['AgentMemoryLevelSchema', AgentMemoryLevelSchema, 'global', 'local', 'tactical'],
      ['LLMProviderSchema', LLMProviderSchema, 'deepseek', 'gpt-4o-mini'],
      ['LegalGroundSourceSchema', LegalGroundSourceSchema, 'platform_rules', 'gk_rf', 'uk_rf'],
      [
        'AppealRecommendationActionSchema',
        AppealRecommendationActionSchema,
        'approve',
